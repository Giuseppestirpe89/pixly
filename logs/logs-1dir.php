...
<?php
  ...
  * A administrator can the decide if it is a error or attacker and blacklist that IP from the server on review of these logs
  * ------------------------------------------------------------------------------------------------------------------------------------------------------------------
  */

    $d = date('Y-m-d-H:i:s');
    $myfile = fopen( "../logs/records/".$d."-regex.php", "w") or die("Unable to open file!");
    $txt = "<br>" .
        "<b>Date:</b> " . $d ."<br>" ."\n".
